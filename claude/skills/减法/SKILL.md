...
- **反思机制 / 经验仓库**：让协议文档变成"持续记账系统"（每次任务后追加一条 lesson 的滚动文件），必膨胀。真要学，外置短日志、只在重复失败时记、定期删。
...
只管上下文文档（skill / AGENTS.md / CLAUDE.md / 系统提示等）。测试覆盖、错误处理、依赖管理是别的 skill 的事，本 skill 不吞。
