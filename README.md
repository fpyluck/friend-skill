...
"朋友"是一个富哥专用的，跨 Agent 协作 skill，同时安装到 **Claude Code** 和 **Codex CLI** 两端。功能1：当任务进入需要决策的阶段（制定执行性计划、上下文压力大、复杂歧义、高风险信号词、破坏性操作），双方会自动用统一协议互相协商；最多 5 轮内达成一致就执行，无法达成就升级给用户裁决。双倍token，双倍快乐，双倍效率，让你的token像流水一样消失。功能2：当CodeX或者ClaudeCode 得到新技能时，朋友skill会迅速把技能推给对方，一个学会，两个全会。
